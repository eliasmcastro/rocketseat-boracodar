  ...
  products.map((product) => {
    ...
    productCard.innerHTML = `
      <div class="product-image">
        <img src="./assets/${product.preview}" alt="${product.alt}" />
      </div>
  ...
  productsName.forEach((productName) => {
    ...
    if (text.length > limit) {
      const productNameFormatted =
        text.substring(0, limit - dots.length) + dots;
...
const secondsTofinishLoading = 1500;
const secondsToMessageDisappear = 3000;

checkout.addEventListener('click', () => {
  const successfulMsg = document.querySelector('#successful-message');

  checkout.innerHTML = `<i class="ph ph-spinner-gap"></i>`;
